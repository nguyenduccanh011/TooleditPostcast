...
from add_shape_bar_impl import add_shape_bar_impl
...
from settings.local import DRAFT_DOMAIN, PREVIEW_ROUTER, PORT, DRAFT_FOLDER
...
def _is_international_font_key(name: str) -> bool:
    # Keep ASCII-only keys for international mode, exclude CJK key names.
    return bool(name) and all(ord(ch) < 128 for ch in name) and bool(re.search(r"[A-Za-z]", name))


def _pick_first_existing_font(candidates: list[str]) -> str:
    for key in candidates:
        if key in Font_type.__members__:
            return key
    # Final fallback: any ASCII key if available, else first member.
    for key in Font_type.__members__.keys():
        if _is_international_font_key(key):
            return key
    return next(iter(Font_type.__members__.keys()))


def _default_font_for_locale() -> str:
    """Return a locale-aware default font key existing in Font_type."""
    locale = (get_language() or "").strip().lower()
    if locale == "zh":
        return _pick_first_existing_font(["SourceHanSansCN_Regular", "SourceHanSansTW_Regular", "Poppins_Regular"])
    # International defaults for en/vi.
    return _pick_first_existing_font(["Poppins_Regular", "Inter_Black", "Nunito"])
...
def add_text():
    ...
    transform_x = data.get('transform_x', 0)
    font = data.get('font', _default_font_for_locale())
    font_color = data.get('color', data.get('font_color', "#FFFFFF"))  # Support both 'color' and 'font_color'
    ...
    subtitle_group_id = data.get('subtitle_group_id', "")
...
def save_draft():
    ...
    # Get required parameters
    draft_id = data.get('draft_id')
    draft_folder = data.get('draft_folder') or DRAFT_FOLDER  # Fallback to auto-detected folder
    ...
    try:
        ...
        if isinstance(draft_result, dict) and draft_result.get("success") is False:
            result["success"] = False
            result["error"] = draft_result.get("error", "save_draft failed")
            result["output"] = draft_result
            return jsonify(result)
...
@app.route('/add_shape_bar', methods=['POST'])
def add_shape_bar():
    data = request.get_json()
    draft_id = data.get('draft_id')
    start = data.get('start', 0)
    ...
    width = data.get('width', 1080)
    height = data.get('height', 1920)
    bar_height_px = data.get('bar_height_px', 100)
    transform_x = data.get('transform_x', 0.0)
    transform_y = data.get('transform_y', 0.0)
    track_name = data.get('track_name', 'shape_bar')
    relative_index = data.get('relative_index', 0)
    ...
    try:
        draft_result = add_shape_bar_impl(
            draft_id=draft_id,
            start=start,
            end=end,
            width=width,
        ...
        result["error"] = f"Error occurred while adding shape bar: {str(e)}."
        return jsonify(result)
...
def get_audio_effect_types():
    ...
    try:
        ...
        # Voice filters effect types
        for name, member in CapCut_Voice_filters_effect_type.__members__.items():
            params_info = [{"name": p.name, "default_value": p.default_value * 100, "min_value": p.min_value * 100, "max_value": p.max_value * 100} for p in member.value.params]
            audio_effect_types.append({"name": name, "type": "Voice_filters", "params": params_info})
        
        # Voice characters effect types
...
def get_font_types():
    ...
    try:
        font_types = []
        intl_only = _to_bool(request.args.get("intl_only"), default=True)
        with_meta = _to_bool(request.args.get("with_meta"), default=False)
        
        # Return font types in current metadata set.
        for name, member in Font_type.__members__.items():
            if intl_only and not _is_international_font_key(name):
                continue

            item = {"name": name}
            if with_meta:
                item["display_name"] = member.value.name
                item["is_vip"] = member.value.is_vip
                item["resource_id"] = member.value.resource_id
                item["effect_id"] = member.value.effect_id
                item["md5"] = member.value.md5
            font_types.append(item)

        font_types.sort(key=lambda x: x["name"])
