...
from downloader import download_audio, download_file, download_audio_file, download_image, download_video
...
def build_asset_path(draft_folder: str, draft_id: str, asset_type: str, material_name: str) -> str:
    ...
    if is_windows_path(draft_folder):
        if os.name == 'nt': # 'nt' for Windows
            ...
            draft_real_path = windows_drive + '\\' + '\\'.join(parts + [draft_id, "assets", asset_type, material_name])
    else:
        draft_real_path = os.path.join(draft_folder, draft_id, "assets", asset_type, material_name)
    ...
    try:
        ...
        if audios:
            for audio in audios:
                ...
                # Add audio download task - use download_audio_file for proper MP3 transcoding
                download_tasks.append({
                    'type': 'audio',
                    'func': download_audio_file,
            ...
            with ThreadPoolExecutor(max_workers=16) as executor:
                ...
                for future in as_completed(future_to_task):
                    ...
                    try:
                        ...
                        failed_types = task.get('type', 'unknown')
                        failed_url = task['args'][0] if task.get('args') else 'unknown'
                        failed_dest = task['args'][1] if task.get('args') and len(task['args']) > 1 else 'unknown'
                        logger.error(f"Task {task_id}: FAILED to download {failed_types} from {failed_url} to {failed_dest}. This WILL cause 'media not found' in CapCut.")
        ...
        # Verify all downloaded files exist before saving draft
        missing_files = []
        for task in download_tasks:
            dest_path = task['args'][1] if len(task['args']) > 1 else None
            if dest_path and not os.path.exists(dest_path):
                missing_files.append(f"{task['type']}: {dest_path}")
            elif dest_path and os.path.getsize(dest_path) == 0:
                missing_files.append(f"{task['type']} (empty): {dest_path}")
        
        if missing_files:
            logger.error(f"Task {task_id}: {len(missing_files)} media files are missing after download:")
            for mf in missing_files:
                logger.error(f"  - {mf}")
            update_task_field(task_id, "message", f"Warning: {len(missing_files)} media files failed to download")
                ...
                if os.path.exists(source_path):
                    ...
                    if os.path.exists(dest_path):
                        try:
                            ...
                            # CapCut may lock files, try removing all except .locked
                            for item in os.listdir(dest_path):
                                ...
                                try:
                                    ...
                                    pass
                            logger.info(f"Cleaned existing draft at {dest_path} (skipped locked files)")
                        else:
                            logger.info(f"Removed existing draft at {dest_path}")
                    
                    # Move or copy draft
                    if os.path.exists(dest_path):
                        # Destination still exists (locked files), copy contents over
...
def save_draft_impl(draft_id: str, draft_folder: str = None) -> Dict[str, str]:
    ...
    try:
        ...
        if status != "completed":
            msg = task_status.get("message", f"save_draft task ended with status={status}")
            logger.error(f"Task {task_id} did not complete successfully: status={status}, message={msg}")
            return {
                "success": False,
                "error": msg,
