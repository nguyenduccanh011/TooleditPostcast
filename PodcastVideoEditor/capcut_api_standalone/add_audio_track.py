...
from pyJianYingDraft import exceptions, CapCut_Voice_filters_effect_type, CapCut_Voice_characters_effect_type, CapCut_Speech_to_song_effect_type, trange
...
def add_audio_track(
    ...
    if draft_folder:
        if is_windows_path(draft_folder) and os.name == 'nt':
            # On Windows, os.path.join handles drive letters correctly
            draft_audio_path = os.path.join(draft_folder, draft_id, "assets", "audio", material_name)
        elif is_windows_path(draft_folder):
            # Running on non-Windows but targeting a Windows path
            windows_drive, windows_path = re.match(r'([a-zA-Z]:)(.*)', draft_folder).groups()
            parts = [p for p in windows_path.split('\\') if p]
            draft_audio_path = windows_drive + '\\' + '\\'.join(parts + [draft_id, "assets", "audio", material_name])
        else:
    ...
    if sound_effects:
        for effect_name, params in sound_effects:
            ...
            # Look for effects in CapCut effect types
            try:
