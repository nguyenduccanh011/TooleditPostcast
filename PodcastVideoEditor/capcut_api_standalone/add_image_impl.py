...
def _resolve_capcut_mask_type(mask_name: str):
    """Resolve mask type by enum member name (case-insensitive) plus common aliases."""
    if not mask_name:
        raise ValueError("Mask type cannot be empty")

    raw = str(mask_name).strip()
    if not raw:
        raise ValueError("Mask type cannot be blank")

    # Common aliases users may pass from UI/exporters.
    alias_map = {
        "linear": "Split",
        "split": "Split",
        "mirror": "Filmstrip",
    ...
    target_name = alias_map.get(raw.lower(), raw)

    # Exact member name first.
    if target_name in draft.CapCut_Mask_type.__members__:
        return draft.CapCut_Mask_type.__members__[target_name]

    # Case-insensitive fallback on actual enum member names.
    for name, member in draft.CapCut_Mask_type.__members__.items():
        if name.lower() == target_name.lower():
            return member

    supported = ", ".join(draft.CapCut_Mask_type.__members__.keys())
    raise ValueError(f"Unsupported mask type '{mask_name}'. Supported types: {supported}")
...
def add_image_impl(
    ...
    if mask_type:
        try:
            mask_type_enum = _resolve_capcut_mask_type(mask_type)
            ...
            raise ValueError(f"Failed to apply mask '{mask_type}': {e}")
